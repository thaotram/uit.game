...
void Object_Unit_Aladdin::ObjectUpdateEvent(float dt) {
	...
	mPos.GetX()->mVelocity = 0;

	mPos.GetY()->mType = Type::quad;
	...
	if (S == "stand") {
		...
		ef (C) 		mAni.Set("stand_jump_up", 1, "stand", 1);
	}
	ef (S == "stand_jump_up") {
		if (!isJumpUp) {
			isJumpUp = true;
	...
	ef (S == "up") {
		...
		ef (U && C)		mAni.Set("stand_jump_up", 1, "stand", 1);
	}
	ef (S == "up_cut") {
	...
	ef (S == "sit") {
		...
		ef (D && C)		mAni.Set("stand_jump_up", 1, "sit", 1);
	}
	ef (S == "run") {
		...
		ef (C)			mAni.Set("run_jump_up", 1, "run", 1);
	}
	ef (S == "run_cut") {
		if (!R && !L)	mAni.Set("stand", 1);
		ef (R)			mPos.GetX()->operator+=(speed * dt);
